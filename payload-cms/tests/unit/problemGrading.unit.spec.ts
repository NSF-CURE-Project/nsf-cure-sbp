...
describe("problemGrading", () => {
  ...
  it("grades fbd-draw parts by angle/magnitude rubric and extra-force penalty", async () => {
    const graded = await gradeProblemAttemptAnswers(
      [
        {
          problem: "fbd-1",
          parts: [
            {
              ...
              placedForces: {
                forces: [
                  { id: "s1", origin: [100, 100], angle: 90, magnitude: 1, label: "N" },
                  { id: "s2", origin: [100, 100], angle: 180, magnitude: 0.95, label: "F" },
                  { id: "extra", origin: [100, 100], angle: 0, magnitude: 1, label: "X" },
                ],
              },
            },
          ],
    ...
    expect(graded.maxScore).toBe(1);
    expect(graded.correctCount).toBe(0);
    expect(graded.score).toBe(0.5);
    expect(graded.answers[0]?.parts[0]?.placedForces?.forces).toHaveLength(3);
  });
});
