  ...
  hooks: {
      ...
      async ({ data, req, originalDoc }) => {
        ...
        if (lessonId) {
          ...
          if (lesson) {
            ...
            data.chapter = (lesson as { chapter?: string | { id?: string } }).chapter ?? data.chapter;
            const chapterId =
            ...
            if (chapterId) {
              ...
              if (chapter) {
                ...
                data.class =
                  (chapter as { class?: string | { id?: string } }).class ?? data.class;
              }
            }
          }
        }
        ...
        if (Array.isArray(data.answers)) {
          ...
          const staffAuthor =
            req.user?.collection === "users" ? req.user.id : undefined;
    ...
    afterChange: [
      async ({ doc, previousDoc, req }) => {
        ...
        const previousAnswers = Array.isArray(previousDoc?.answers)
          ? previousDoc.answers.length
          : 0;
        const nextAnswers = Array.isArray(doc.answers)
          ? doc.answers.length
          : 0;
        const hasNewAnswer = nextAnswers > previousAnswers;
        const becameAnswered =
          doc.status === "answered" && previousDoc?.status !== "answered";
