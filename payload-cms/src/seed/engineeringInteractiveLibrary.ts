...
export default async function seedEngineeringInteractiveLibrary(payload: Payload) {
  ...
  const ringForcesFigure = await upsertFigure(payload, FIGURE_TITLES.ringForces, {
    ...
    figureData: {
      ...
      forces: [
        { id: "F1", label: "F1 = 7", origin: [330, 200], angle: 20, magnitude: 1.1, color: "#2563eb" },
        { id: "F2", label: "F2 = 9", origin: [330, 200], angle: 140, magnitude: 1.2, color: "#ef4444" },
        { id: "F3", label: "F3 = 5", origin: [330, 200], angle: 270, magnitude: 0.9, color: "#16a34a" },
      ],
    },
  });
  ...
  const inclinedMomentFigure = await upsertFigure(payload, FIGURE_TITLES.inclinedMoment, {
    ...
    figureData: {
      ...
      forces: [{ id: "F", label: "F", origin: [390, 180], angle: 120, magnitude: 1.2, color: "#ef4444" }],
  ...
  const partialUdlBeamFigure = await upsertFigure(payload, FIGURE_TITLES.partialUdlBeam, {
    type: "beam",
    description: "Simply supported beam with a partial uniformly distributed load over the middle span.",
    width: 680,
    height: 400,
    figureData: {
      type: "beam",
  ...
  const axialBarFigure = await upsertFigure(payload, FIGURE_TITLES.axialBar, {
    ...
    figureData: {
      ...
      dimensions: [
        { from: [180, 250], to: [480, 250], label: "L" },
      ],
    },
  });
  ...
  const partialUdlProblem = await upsertProblem(payload, PROBLEM_TITLES.partialUdlBeam, {
    ...
    parts: [
      ...
      {
        label: "W_expr",
        prompt: richText("Enter expression for equivalent load in terms of w, a, and b (loaded from a to b)."),
        partType: "symbolic",
  ...
  const allProblemIds = [...staticsSet1Ids, ...staticsSet2Ids, ...staticsSet3Ids, ...mechanicsSetIds];
