...
const toListBlock = (
  ...
  listStyle: 'ordered' | 'unordered' = 'unordered',
  title?: string,
) => {
...
const isLayoutEmpty = (layout?: unknown) => !Array.isArray(layout) || layout.length === 0
...
export default async function migrateLayout(payload: Payload) {
  ...
  if (home && isLayoutEmpty((home as any).layout)) {
    ...
    const gsSteps = Array.isArray((home as any).gettingStartedSteps)
      ? (home as any).gettingStartedSteps
          .map((s: any) => ({ text: s?.step ?? '' }))
          .filter((s: any) => s.text)
      : []
  ...
  if (resources && isLayoutEmpty((resources as any).layout)) {
    ...
    const hero = toSectionTitle(
      (resources as any).heroTitle ?? '',
      (resources as any).heroIntro ?? null,
    )
    if (hero) blocks.push(hero)
  ...
  if (contact && isLayoutEmpty((contact as any).layout)) {
    const blocks: LayoutBlock[] = []
    const hero = toSectionTitle(
      (contact as any).heroTitle ?? '',
      (contact as any).heroIntro ?? null,
    )
    if (hero) blocks.push(hero)
  ...
  if (gettingStarted && isLayoutEmpty((gettingStarted as any).layout)) {
    ...
    const steps = Array.isArray((gettingStarted as any).steps) ? (gettingStarted as any).steps : []
    if (steps.length) {
      blocks.push({
        blockType: 'stepsList',
        title: 'Steps',
